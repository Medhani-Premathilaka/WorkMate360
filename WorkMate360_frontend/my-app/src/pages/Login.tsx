    ...
    <div className="bg-[url(../assets/images/login.jpg)] min-h-screen w-full bg-cover bg-center">
      <div className="fixed bottom-1/2 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-80 h-auto p-8 border-2 border-white/50 bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl font-serif">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <div className="mb-4">
            <label htmlFor="username" className='block text-slate-600 mb-1'>Username</label>
            <input 
              type="text" 
              name="username"
              id="username"
          ...
          <button 
            type="submit" 
            className={`w-full py-2 px-4 rounded-2xl text-white ${isLoading ? 'bg-slate-400' : 'bg-slate-700 hover:bg-slate-600'} transition-colors`}
            disabled={isLoading}
          >
            {isLoading ? (
