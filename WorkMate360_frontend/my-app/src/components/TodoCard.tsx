...
const handleDescription = () =>{
  ...
  const handleToggleComplete = async (todo: Todo) => {
    try {
      ...
      await axios.put(`http://localhost:8080/todo/update/${todo.id}`, updatedTodo);
  ...
  return (
    <div className="fixed top-70 left-90 shadow-[0_35px_35px_rgba(0,0,0,0.25)] rounded-2xl">
        <div className="flex flex-col items-center justify-center p-10 text-red-600">
          ...
          <span className="text-lg font-semibold mb-2">Failed to load Todos</span>
          <span className="text-base">{error}</span>
          <p className="mt-2 text-sm text-gray-500">
            Please ensure your backend server is running and accessible at <span className="font-mono">http://localhost:8080/todo/getByProfileId</span>.<br />
        ...
        {todos.map((todo) => (
          <React.Fragment key={todo.id}>
            <div 
              ...
              className={`p-4 border rounded-lg shadow-md flex justify-between items-center hover:scale-102 ${
                todo.isCompleted ? 'bg-gray-50 border-gray-200' : 'bg-white border-slate-200'
              }`}
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-grow">
                  <h3 className={`text-lg font-medium ${todo.isCompleted ? 'line-through text-gray-500' : 'text-slate-700'}`}>
                    {todo.title}
                  </h3>
            ...
            {selectedTodoId === todo.id && (
              <div className="mt-3 p-3 bg-slate-50 rounded text-slate-700 border border-slate-200">
                ...
                <div>{(todo as any).description || <span className="italic text-gray-400">No description</span>}</div>
              </div>
            )}
          </React.Fragment>
        ))}
